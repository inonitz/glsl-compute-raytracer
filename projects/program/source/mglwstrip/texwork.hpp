...
{
	util::math::vec3u localGroup;
	util::math::vec3u dispatchGroup;
};
...
ComputeGroupSizes computeDispatchSize(util::math::vec2u const& dims);
void renderUI(
...
inline ComputeGroupSizes computeDispatchSize(util::math::vec2u const& dims)
{
	util::math::vec3u localWorkgroupSize{32, 1, 1};
	i32 		max_group_invoc;
	util::math::vec2f tmp_cvt, tmp_cvt1;


	gl::glGetIntegerv(gl::GL_MAX_COMPUTE_WORK_GROUP_INVOCATIONS, &max_group_invoc);
